...
| **Item Type** | The category of an item. lodging (overnight stay), transportation (movement between places — can bridge phases), flight (air travel, has AeroDataBox integration), activity (things to do), meal (dining), note (free text), checklist (has sub-items). | category |
...
| **Trip Goal** | *(v4 — not yet built.)* A trip-level aspiration that isn't specific enough to be an item: "try paella," "do a wine tasting." No phase, location, or time. One or more items can address a goal. Enables a quick-planning minigame for capturing group wishes early. | wish, bucket list item |
...
| **Vote** | A member's preference on an item. One vote per member per item. Four options: Love (+2), Like (+1), Flexible (0), Dislike (-2). Score is never shown numerically — only the relative order and who voted what (avatar stacks). Unplanned items sort by aggregate vote score. | rating, star, like |
...
| **Public Archive** | Read-only, token-gated view of a closed trip. PII stripped. Published after configurable delay. | shared link |

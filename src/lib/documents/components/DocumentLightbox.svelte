			...
			<div class="flex shrink-0 items-center gap-2">
				{#if shareSupported}
					<button
						...
						onclick={share}
						disabled={sharing}
						class="hit-44 rounded-md px-2.5 py-1.5 text-xs font-semibold text-paper/90 hover:bg-paper/10 active:bg-paper/10 disabled:opacity-40"
					>
						Share
					</button>
				{/if}
				<a href={doc.file_href} target="_blank" rel="noopener" class="hit-44 rounded-md px-2.5 py-1.5 text-xs font-semibold text-paper/90 hover:bg-paper/10 active:bg-paper/10">Open</a>
				<a href="{doc.file_href}?download=1" download class="hit-44 rounded-md px-2.5 py-1.5 text-xs font-semibold text-paper/90 hover:bg-paper/10 active:bg-paper/10">Save</a>
				{#if deletable}
					{#if !confirming}
						<button type="button" onclick={() => (confirming = true)} class="hit-44 rounded-md px-2.5 py-1.5 text-xs font-semibold text-paper/90 hover:bg-paper/10 active:bg-paper/10">Delete</button>
					{:else}
						<form
							method="POST"
							action={deleteAction}
						...
						>
							...
							<button type="submit" disabled={deleting} class="hit-44 bg-error text-paper rounded-md px-2.5 py-1.5 text-xs font-semibold disabled:opacity-40">
								{deleting ? 'Deleting…' : 'Confirm'}
							</button>
							<button type="button" onclick={() => (confirming = false)} class="hit-44 text-paper/70 px-1.5 text-xs">Cancel</button>
						</form>
					{/if}
				{/if}
				<button type="button" onclick={close} class="hit-44 flex h-8 w-8 items-center justify-center rounded-md text-paper/90 hover:bg-paper/10 active:bg-paper/10" aria-label="Close">
